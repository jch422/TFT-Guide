...
module.exports = {
  getTraits: async (req, res) => {
    const { champions: champion } = req.body;
    const data = {};
    const recommend = [];
    ...
    for (let i = 0; i < traitData.length; i++) {
      ...
      for (let j = 0; j < traitData[i].length; j++) {
        ...
        for (let k = 0; k < traitData[i][j].length; k++) {
          ...
          championSynergy[i] = {
            key: traitData[i][1],
            sets: traitData[i][2][temp],
            num: data[`${traitData[i][1]}`],
          };
        }
      }
    }
    res.status(200).json({ data: championSynergy });
  },
  getRecommend: async (req, res) => {
    ...
    for (let cost in champion_score) {
      ...
      sortobj.push([...filterChampion, champion_score[cost]]);
    }
    sortobj.sort(function (a, b) {
      return b[1] - a[1];
    });
    const lastData = sortobj.slice(1, 6);
